...
def build_place(mail: gmail_client.Mail) -> maps_resolver.Place | None:
    """メールから店を特定する。特定できなければ None。"""
    maps_url = maps_resolver.pick_maps_url(mail.urls)
    if maps_url:
        log(f"  Google マップ: {maps_url}")
        place = maps_resolver.resolve(maps_url)
        if place.name:
            return place
        log("  マップから店名を取れなかった")

    # マップ URL が無い場合、件名を店名とみなす（README の運用どおり）。
    # 住所は裏が取れないので None のまま = 「未確認」。
    if mail.subject.strip():
        return maps_resolver.Place(name=mail.subject.strip())
    return None
...
def main() -> int:
    ...
    for thread_id in thread_ids:
        ...
        if place.address or place.hours:
