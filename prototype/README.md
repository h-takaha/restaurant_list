...
1軒に絞れなかったとき（URL が `/place/` に移らない）と、出てきた店名が件名と似ていない
ときは、**選ばずに受信箱へ残す。**「すすきのの店」という件名で無関係な店の住所を
書いてしまうのが最悪なので、同一性は文字バイグラムの重なりで機械的に確かめる。

**4. LLM に知識を思い出させない。** 住所を「調べさせる」と小型モデルは平然と捏造する。
`tagger.py` に渡すのは、マップと公式サイトから取ってきたテキストだけ。そこからの分類に
閉じているので、ローカルの 8B クラスでも成立する。`ClaudeTagger` と `OllamaTagger` は
入出力が同じで差し替えられる。
...
どちらもネットワークに出ない。合わせて 86 項目。

```bash
python test_extraction.py     # 部品の抽出ロジック（41）
python test_pipeline.py       # run.py の判断ロジック（45）
```
...
- 店名の同一性判定（表記ゆれを吸収し、別の店は弾く）と検索 URL の組み立て
...
- **実物の Google マップに対しては未検証。** 開発環境の egress ポリシーで google.com に
  ...
  変わるが、腐れば住所が取れず受信箱に溜まるので、気づける壊れ方にはなっている
...
- **`tagger.py` の `confident` 判定**と **`name_matches` の閾値（0.3）**は未チューニング。
  実データで見る必要がある
